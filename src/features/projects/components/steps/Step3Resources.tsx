...
export default function Step3Resources() {
  console.log(
    'src/features/projects/components/steps/Step1GeneralInfo.tsx/Step3Resources'
  );
  ...
  const {
    ...
    append: appendSkill,
    remove: removeSkill,
  } = useFieldArray({ control, name: 'data.resources.skillMatrix' });
      ...
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6 shadow-sm space-y-6">
        ...
        <div className="grid md:grid-cols-2 gap-5">
          <div>
            ...
            <Input
              type="number"
              {...register('data.resources.teamSize', { valueAsNumber: true })}
            />
            {errors?.data?.resources?.teamSize?.message && (
          ...
          <div>
            ...
            <Input
              ...
              {...register('data.resources.departments')}
            />
            {errors?.data?.resources?.departments?.message && (
              <p className="text-sm text-red-500 mt-1">
                {errors.data.resources.departments.message}
              </p>
            )}
          ...
          <div>
            ...
            <Input
              ...
              {...register('data.resources.roles')}
            />
            {errors?.data?.resources?.roles?.message && (
              <p className="text-sm text-red-500 mt-1">
                {errors.data.resources.roles.message}
              </p>
            )}
          ...
          <div>
            ...
            <Input
              ...
              {...register('data.resources.budget')}
            />
            {errors?.data?.resources?.budget?.message && (
              <p className="text-sm text-red-500 mt-1">
                {errors.data.resources.budget.message}
              </p>
            )}
            ...
            <div key={field.id} className="grid md:grid-cols-4 gap-4 items-end">
              <div>
                ...
                <Input
                  ...
                  {...register(`data.resources.skillMatrix.${index}.skill`)}
                />
                {errors?.data?.resources?.skillMatrix?.[index]?.skill?.message && (
                  <p className="text-sm text-red-500 mt-1">
                    {errors.data.resources.skillMatrix[index].skill.message}
                  </p>
                )}
              ...
              <div>
                ...
                <Input
                  type="number"
                  {...register(`data.resources.skillMatrix.${index}.required`, {
                    valueAsNumber: true,
                  })}
                />
              </div>
              ...
              <div>
                ...
                <Input
                  type="number"
                  {...register(
                    `data.resources.skillMatrix.${index}.available`,
                    {
                      valueAsNumber: true,
                    }
                  )}
        ...
        <div className="space-y-4 border-t pt-4">
          ...
          <EditableEmployeeGrid
            ...
            // onChange={(rows) =>
            //   setValue('data.resources.extendedEmployees', rows)
            // }
          />
        </div>
