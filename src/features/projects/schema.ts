...
export const generalInfoSchema = z.object({
  name: z.string().optional(), //.min(2, 'Project name must be at least 2 characters'),
  sponsor: z.string().optional(), //.min(2, 'Sponsor name must be at least 2 characters'),
  type: z.enum(['Internal', 'External'], {
    required_error: 'Project type is required',
  }),
  description: z.string().optional(),
  code: z.string().optional(), //.min(3, 'Project code must be at least 3 characters'),
  businessUnit: z.string().optional(), //.min(1, 'Business Unit is required'),
  confidentiality: z
...
const phaseSchema = z.object({
      label: z.string().optional(), //.min(1, 'Phase name is required'),
      from: z.string().optional(), //.min(1, 'Start date is required'),
      to: z.string().optional(), //.min(1, 'End date is required'),
    });
const milestoneSchema = z.object({
      title: z.string().optional(), //.min(1, 'Milestone title is required'),
      deadline: z.string().optional(), //.min(1, 'Deadline is required'),
      owner: z.string().optional(), //.min(1, 'Owner is required'),
    })
export const timelineSchema = z.object({
  startDate: z.string().optional(), //.min(1, 'Start date is required'),
  endDate: z.string().optional(), //.min(1, 'End date is required'),
...
const allocationLogSchema = z.object({
  timestamp: z.string(),
  phase: z.string().optional(), //.min(1, 'Phase is required'),
  allocation: z.number().optional(), //.min(0).max(100),
  status: z.enum(['Planned', 'Confirmed', 'Rejected']),
  reviewer: z.string().optional(), //.min(1, 'Reviewer is required'),
});
...
const extendedEmployeeSchema = z.object({
  ...
  name: z.string().optional(), //.min(1, 'Name is required'),
  ...
  // }),
  allocationLogs: z.array(allocationLogSchema).optional(),
...
const skillSchema = z.object({
  skill: z.string().optional(), //.min(1, 'Skill is required'),
  required: z.number().optional(), //.min(0),
  available: z.number().optional(), //.min(0),
});

// Entire resources block
export const resourcesSchema = z.object({
  teamSize: z.number().optional(), //.min(1, 'Team size is required'),
  departments: z.string().optional(), //.min(1, 'At least one department is required'),
  roles: z.string().optional(),
  budget: z.string().optional(), //.min(1, 'Budget is required'),
...
export const requirementsSchema = z.object({
  complianceNeeds: z.boolean().optional(),
  ...
  integrationTypes: z
    .array(
      z.string()//.optional() //.min(1, 'Integration type is required')
    ).optional().default([])
    // .min(0)
    ,
...
export type ExtendedEmployee = z.infer<typeof extendedEmployeeSchema>;
export type Phase = z.infer<typeof phaseSchema>;
export type Milestone = z.infer<typeof milestoneSchema>;
export type AllocationLog = z.infer<typeof allocationLogSchema>;
export type RequirementsForm = FieldValues & ProjectFormData['data']['requirements'];

export const INITIAL_DATA: ProjectFormData = {
  projectId: uuidv4(),
  status: 'draft',
  isArchived: false,
  createdBy: '', // To be set from auth
...
export type ProjectEntry = {
  userId: string; // user ID or project ID, depending on your schema
  projects: {
    projectId: string;
